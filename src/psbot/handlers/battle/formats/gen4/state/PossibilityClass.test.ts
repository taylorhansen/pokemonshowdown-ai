...
export const test = () =>
    describe("PossibilityClass", function () {
        ...
        describe("#remove()", function () {
            ...
            it("Should throw if overnarrowed", function () {
                const pc = new PossibilityClass(map);
                expect(() => pc.remove("a", "b", "c")).to.throw(
                    Error,
                    "All possibilities have been ruled out " +
                        "(should never happen)",
                );
        ...
        describe("#onNarrow()", function () {
            ...
            it("Should not be called more than once", function () {
                ...
                expect(count).to.equal(
                    1,
                    `Expected to be called once but got ${count} times`,
                );
            });
        ...
        describe("#onUpdate()", function () {
            ...
            it("Should call with kept=true when narrowed down to a subset", function () {
                const pc = new PossibilityClass(map);
                let kept: boolean | null = null;
            ...
            it("Should call with kept=false when entire subset is ruled out", function () {
                const pc = new PossibilityClass(map);
                let kept: boolean | null = null;
                pc.onUpdate(new Set(["a", "b"]), k => (kept = k));
                expect(kept).to.be.null;
            ...
            it("Should call immediately with kept=false when subset over-narrowed", function () {
                const pc = new PossibilityClass(map);
                let kept: boolean | null = null;
                pc.remove("a");
                pc.onUpdate(new Set("a"), k => (kept = k));
            ...
            it("Should call immediately with kept=true when already narrowed", function () {
                const pc = new PossibilityClass(map);
                pc.narrow("a");
                let kept: boolean | null = null;
                pc.onUpdate(new Set("a"), k => (kept = k));
            ...
            it("Should have same behavior if predicate and kept callback are inverted", function () {
                const pc = new PossibilityClass(map);
