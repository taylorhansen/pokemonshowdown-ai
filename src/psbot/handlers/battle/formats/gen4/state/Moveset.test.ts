    ...
    describe("Moveset", function () {
        ...
        describe("#inferDoesntHave()", function () {
            ...
            it("Should narrow move slot constraints", function () {
                ...
                expect(moveset.moveSlotConstraints[0]).to.have.keys(
                    "tackle",
                    "gigaimpact",
                );
            });
            ...
            it("Should throw if the call would over-narrow move slot constraint", function () {
                const moveset = new Moveset(
                    [
                        "splash",
                        "tackle",
                ...
                expect(() =>
                    moveset.inferDoesntHave(["splash", "tackle"]),
                ).to.throw(
                    Error,
                    "Rejected Moveset#inferDoesntHave() with " +
                        "moves=[splash, tackle] since the Moveset's slot " +
                        "constraint [splash, tackle] would be invalidated",
                );
            });
        });
        ...
        describe("#addMoveSlotConstraint()", function () {
            it("Should add move slot constraint", function () {
                ...
                expect(moveset.moveSlotConstraints).to.have.lengthOf(1);
                expect(moveset.moveSlotConstraints[0]).to.have.all.keys(
                    fourMoves,
                );
            });
            ...
            it("Should reveal if intersection with movepool yields one move", function () {
                const moveset = new Moveset(twoMoves, 1);
                expect(moveset.get("tackle")).to.be.null;
            ...
            it("Should throw if intersection with movepool is empty", function () {
                const moveset = new Moveset(twoMoves, 1);
                expect(() =>
                    moveset.addMoveSlotConstraint(["takedown"]),
                ).to.throw(
                    Error,
                    "Move slot constraint [takedown] cannot " +
                        "exist for this Moveset",
                );
            });

            it("Should intersect all move slot constraints if one move left", function () {
                const moveset = new Moveset(fourMoves, 2);
            ...
            it("Should throw if one move left and move slot constraints can't intersect", function () {
                const moveset = new Moveset(2);
                moveset.addMoveSlotConstraint(["tackle", "splash"]);
                moveset.addMoveSlotConstraint(["hyperbeam", "gigaimpact"]);

                expect(() => moveset.reveal("takedown")).to.throw(
                    Error,
            ...
            it("Should not throw if already satisfied and Moveset is full", function () {
            ...
            it("Should throw if not already satisfied and Moveset is full", function () {
                const moveset = new Moveset(fourMoves);
                expect(() =>
                    moveset.addMoveSlotConstraint(["metronome", "flash"]),
                ).to.throw(
                    Error,
                    "Move slot constraint [metronome, flash] cannot exist " +
                        "for this Moveset",
                );
            });
        });
