...
export const test = () => describe("ItemTempStatus", function()
{
    type StatusType = "a" | "b";
    ...
    describe("#start()", function()
    {
        ...
        it("Should update duration if item was narrowed to extension item",
        function()
        {
            its.start(source, "b");
            expect(its.duration).to.equal(its.durations[0]);
        ...
        it("Should not update duration if item was narrowed differently",
        function()
        {
            its.start(source, "b");
            expect(its.duration).to.equal(its.durations[0]);
    ...
    describe("#tick()", function()
    {
        ...
        it("Should throw if kept past short duration and can't have extension" +
            "item",
        function()
        {
            its.start(source, "a");
            ...
            expect(() => its.tick()).to.throw(Error,
                "Status 'a' went longer than expected " +
                `(duration=${its.durations[0]}, turns=${its.durations[0]})`);
        });
        ...
        it("Should always throw if kept past long duration", function()
        {
            ...
            expect(() => its.tick()).to.throw(Error,
                ...
                `(duration=${its.durations[1]}, turns=${its.durations[1]})`);
        });
    });
});
