 ...
 */
export const onPreMove = onX("onPreMove",
    (ctx, side) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canPreMove(mon));
    },
    onXInferenceParser("onPreMoveInference",
        onXUnorderedParser("onPreMoveUnordered",
 ...
 */
export const onMoveCharge = onX("onMoveCharge",
    (ctx, side) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canMoveCharge(mon));
    },
    onXInferenceParser("onMoveChargeInference",
        onXUnorderedParser("onMoveChargeUnordered",
            async (ctx, accept, item, side) =>
 ...
 * if any.
 */
export const onPreHit = onX("onPreHit",
    (ctx, side, hitBy: dex.MoveAndUser) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canPreHit(mon, hitBy));
    },
    onXInferenceParser("onPreHitInference",
        onXUnorderedParser("onPreHitUnordered",
 ...
 * @returns An EventInference for handling item possibilities.
 */
export const onSuper = onX("onSuper",
    (ctx, side, hitBy: dex.MoveAndUser) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canSuper(mon, hitBy));
    },
    onXInferenceParser("onSuperInference",
        onXUnorderedParser("onSuperUnordered",
            async (ctx, accept, item, side) =>
 ...
 * @returns An EventInference for handling item possibilities.
 */
export const onPostHit = onX("onPostHit",
    ...
    onXInferenceParser("onPostHitInference",
        onXUnorderedParser("onPostHitUnordered",
            async (ctx, accept, item, side, hitBy) =>
 ...
 * @returns An EventInference for handling item possibilities.
 */
export const onUpdate = onX("onUpdate",
    (ctx, side) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canUpdate(mon));
    },
    onXInferenceParser("onUpdateInference",
        onXUnorderedParser("onUpdateUnordered",
            async (ctx, accept, item, side) =>
 ...
 * @returns An EventInference for handling item possibilities.
 */
export const onResidual = onX("onResidual",
    (ctx, side) =>
    {
        const mon = ctx.state.getTeam(side).active;
        return getItems(mon, item => item.canResidual(mon));
    },
    onXInferenceParser("onResidualInference",
        onXUnorderedParser("onResidualUnordered",
            async (ctx, accept, item, side) =>
            ...
            TResult>)
{
        ...
        ...args: TArgs)
    {
        ...
        return new inference.EventInference(`${side} item on-${onString}`,
            new Set(items.values()), inferenceParser, side, items, ...args);
    }}[name];
}
