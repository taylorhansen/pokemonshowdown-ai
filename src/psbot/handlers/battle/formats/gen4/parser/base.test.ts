    ...
    describe("base", function () {
        ...
        describe("|move|", function () {
            it("Should handle move use", async function () {
                ...
                await ph.handle({
                    args: ["-damage", toIdent("p2"), toHPStatus(78)],
                    kwArgs: {},
                });
                await ph.halt();
                await ph.return();
        ...
        describe("|cant|", function () {
            ...
            describe("reason = Focus Punch move", function () {
                it("Should ignore since the |move| context should already be handling this", async function () {
                    const mon = sh.initActive("p1");
                ...
                await ph.rejectError(
                    {
                        args: [
                            "-block",
                            toIdent("p1"),
                            toEffectName("Dynamax"),
                        ],
                        kwArgs: {},
                    },
                    Error,
                    "Unsupported event type |-block|",
                );
            });
        });
        ...
        describe("|-curestatus|", function () {
            ...
            it("Should throw if mentioning an unrelated status", async function () {
                ...
                await ph.rejectError(
                    ...
                    Error,
                    "MajorStatus 'psn' was expected to be 'tox'",
                );
            });
        });
        ...
        describe("|-weather|", function () {
            ...
            describe("ability effect", function () {
                it("Should infer infinite duration if ability matches weather", async function () {
                    const mon = sh.initActive("p2");
                ...
                it("Should reject ability if it doesn't match event", async function () {
                    const mon = sh.initActive("p2");
            ...
            it("Should throw if a different weather is upkept", async function () {
                ...
                await ph.rejectError(
                    weatherEvent("Hail", {upkeep: true}),
                    Error,
                    "Weather is 'SunnyDay' but ticked weather is 'Hail'",
                );
            });
        });
        ...
        describe("|-swapsideconditions|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    {args: ["-swapsideconditions"], kwArgs: {}},
                    Error,
                    "Unsupported event type |-swapsideconditions|",
                );
            });
        });
            ...
            describe(name, function () {
                if (start) {
                    ...
                    it("Should start typechange and reset typeadd", async function () {
                        ...
                        expect(v.overrideTraits!.types).to.have.members([
                            "normal",
                            "???",
                        ]);
                        v.addedType = "bug";
                ...
                for (const effect of [
                    ...
                    it(`Should ${verb} ${effect}`, async function () {
                        ...
                        await ph.handle({
                            args: [eventName, toIdent("p1"), effectStr],
                            kwArgs: {},
                        });
                        await ph.return();
                ...
                for (const [effect, type] of [
                    ...
                    const effectStr = type
                        ? toEffectName(effect, type)
                        : toEffectName(effect);
                    it(`Should ${verb} ${effect}`, async function () {
                        const {volatile: v} = sh.initActive("p1");
                        if (!start) v[effect].start();
                        expect(v[effect].isActive).to.be[
                            start ? "false" : "true"
                        ];
                    ...
                    if (start && effect === "uproar") {
                        it("Should update uproar if upkeep and already active", async function () {
                ...
                if (start) {
                    it("Should throw if starting encore but no lastMove was set", async function () {
                        const {volatile: v} = sh.initActive("p1");
                        expect(v.encore.ts.isActive).to.be.false;
                        expect(v.encore.move).to.be.null;
        ...
        describe("|-enditem|", function () {
            it("Should consume item", async function () {
                ...
                expect(item.possibleValues).to.include.keys(
                    "focussash",
                    "mail",
                );
                ...
                await ph.rejectError(
                    {
                        args: [
                            "-ability",
                            toIdent("p1"),
                            "invalid" as Protocol.AbilityName,
                        ],
                        kwArgs: {},
                    },
                    Error,
        ...
        describe("|-primal|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    {args: ["-primal", toIdent("p1")], kwArgs: {}},
                    Error,
                    "Unsupported event type |-primal|",
                );
            });
        });
        ...
        describe("|-zpower|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    {args: ["-zpower", toIdent("p1")], kwArgs: {}},
                    Error,
                    "Unsupported event type |-zpower|",
                );
            });
        });
        ...
        describe("|-zbroken|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    {args: ["-zbroken", toIdent("p1")], kwArgs: {}},
                    Error,
        ...
        describe("|-activate|", function () {
            ...
            for (const [effect, type] of [
                ...
                it(`Should update ${effect}`, async function () {
                    ...
                    // Then update it.
                    await ph.handle({
                        args: ["-activate", toIdent("p1"), effectStr],
                        kwArgs: {},
                    });
                    await ph.return();
                    expect(v[effect].isActive).to.be.true;
            ...
            for (const effect of [
                ...
                it("Should throw if substitute mentioned but pokemon doesn't have a Substitute", async function () {
                    sh.initActive("p1");
                    await ph.rejectError(
                        {
                            args: [
            ...
            it("Should throw if no lastMove on sketch/mimic", async function () {
                ...
                await ph.rejectError(
                    ...
                    Error,
                    "Don't know how Mimic/Sketch was caused",
                );
            });
            ...
            it("Should throw if invalid lastMove on sketch/mimic", async function () {
                ...
                await ph.rejectError(
                    ...
                    Error,
                    "Unknown Mimic-like move 'invalid'",
                );
            });
        ...
        describe("|-waiting|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    ...
                    Error,
                    "Unsupported event type |-waiting|",
                );
            });
        });
        ...
        describe("|-prepare|", function () {
            ...
            it("Should throw if not a two-turn move", async function () {
                ...
                await ph.rejectError(
                    {
                        args: ["-prepare", toIdent("p2"), toMoveName("ember")],
                        kwArgs: {},
                    },
                    Error,
                    "Move 'ember' is not a two-turn move",
                );
            });
        });
        ...
        describe("|-singlemove|", function () {
            function testSingleMove(
                ...
                it(`Should start ${effect}`, async function () {
                    ...
                    await ph.handle({
                        args: ["-singlemove", toIdent("p1"), moveName],
                        kwArgs: {},
                    });
                    await ph.return();
        ...
        describe("|-candynamax|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    {args: ["-candynamax", "p1"], kwArgs: {}},
                    Error,
                    "Unsupported event type |-candynamax|",
                );
            });
        });
        ...
        describe("|updatepoke|", function () {
            it("Should throw since unsupported", async function () {
                await ph.rejectError(
                    ...
                    Error,
                    "Unsupported event type |updatepoke|",
                );
            });
        });
        ...
        describe("ignoredEvents", function () {
            it("Should consume ignored events", async function () {
                ...
                await ph2.handle({
                    args: ["t:", "time" as Protocol.Timestamp],
                    kwArgs: {},
                });
                await ph2.close();
            });
