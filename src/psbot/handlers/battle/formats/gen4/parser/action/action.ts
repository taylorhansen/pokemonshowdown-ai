...
export async function playerActions(ctx: BattleParserContext<"gen4">) {
    ...
    const switchResults = await unordered.all(
        ctx,
        (Object.entries(ctx.state.teams) as [SideID, unknown][]).map(([side]) =>
            playerSwitchAction(side, actioned),
        ),
        filter,
    );
    Object.assign(actioned, ...switchResults.map(res => res.actioned));

    const moveResults = await unordered.all(
        ctx,
