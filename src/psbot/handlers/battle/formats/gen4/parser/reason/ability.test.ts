...
export const test = () => describe("ability", function()
{
    ...
    for (const [name, has] of [["has", true], ["doesntHave", false]] as const)
    {
        describe(`${name}()`, function()
        {
            ...
            {
                it(`Should return ${has} if having the ability is the only ` +
                ...
                it(`Should return ${!has} if not possible to have the ability`,
                function()
                {
                    const mon = sh.initActive("p1");
            ...
            describe("#assert()", function()
            {
                it(`Should narrow ability and set #canHold()=true`, function()
                {
                    ...
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys("illuminate", "owntempo");

                    const reason = reasonAbility[name](mon,
                        new Set(["illuminate"]));
                    reason.assert();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(has ? "illuminate" : "owntempo");
            ...
            {
                it(`Should narrow ability and set #canHold()=false`,
                function()
                {
                    const mon = sh.initActive("p1");
                    mon.setAbility("illuminate", "owntempo");
                    ...
                    reason.reject();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(has ? "owntempo" : "illuminate");
    ...
    for (const [name, can] of
        [["canIgnoreItem", true], ["cantIgnoreItem", false]] as const)
    {
        describe(`${name}()`, function()
        {
            describe("#canHold()", function()
            {
                it(`Should return ${can} if having an item-ignoring ability ` +
                    "is the only possibility", function()
                {
                    const mon = sh.initActive("p1");
                ...
                it("Should return null if possible to have or not have an " +
                    "item-ignoring ability", function()
                {
                    const mon = sh.initActive("p1");
                ...
                it(`Should return ${!can} if not possible to have an ` +
                    "item-ignoring ability", function()
                {
                    const mon = sh.initActive("p1");
                    mon.setAbility("owntempo");
            ...
            describe("#assert()", function()
            {
                it(`Should narrow ability and set #canHold()=true`, function()
                {
                    ...
                    mon.setAbility("klutz", "owntempo");
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys("klutz", "owntempo");

                    const reason = reasonAbility[name](mon);
                    reason.assert();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(can ? "klutz" : "owntempo");
                    expect(reason.canHold()).to.be.true;
                });
            ...
            describe("#reject()", function()
            {
                ...
                function()
                {
                    ...
                    reason.reject();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(can ? "owntempo" : "klutz");
                    expect(reason.canHold()).to.be.false;
                });
    ...
    for (const [name, can] of [["cantIgnoreTargetAbility", false]] as const)
    {
        describe(`${name}()`, function()
        {
            describe("#canHold()", function()
            {
                it(`Should return ${can} if having a target-ability-ignoring ` +
                    "ability is the only possibility", function()
                {
                    const mon = sh.initActive("p1");
                ...
                it("Should return null if possible to have or not have a " +
                    "target-ability-ignoring ability", function()
                {
                    const mon = sh.initActive("p1");
                ...
                it(`Should return ${!can} if not possible to have a ` +
                    "target-ability-ignoring ability", function()
                {
                    const mon = sh.initActive("p1");
                    mon.setAbility("owntempo");
            ...
            describe("#assert()", function()
            {
                it(`Should narrow ability and set #canHold()=true`, function()
                {
                    ...
                    mon.setAbility("moldbreaker", "owntempo");
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys("moldbreaker", "owntempo");

                    const reason = reasonAbility[name](mon);
                    reason.assert();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(can ? "moldbreaker" : "owntempo");
                    expect(reason.canHold()).to.be.true;
                });
            ...
            describe("#reject()", function()
            {
                ...
                function()
                {
                    ...
                    reason.reject();
                    expect(mon.traits.ability.possibleValues)
                        .to.have.keys(can ? "owntempo" : "moldbreaker");
                    expect(reason.canHold()).to.be.false;
                });
    ...
    describe("targetIgnoring()", function()
    {
        it("Should get target-ignoring abilities", function()
        {
            ...
            expect(reasonAbility.targetIgnoring(mon))
                .to.have.keys("moldbreaker");
        });
