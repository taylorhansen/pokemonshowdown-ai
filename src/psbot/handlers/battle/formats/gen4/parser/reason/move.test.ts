...
export const test = () => describe("move", function()
{
    ...
    describe("diffType()", function()
    {
        ...
        const init = (move: dex.Move) => reasonMove.diffType(mon, {move, user});

        describe("#canHold()", function()
        {
            it("Should return true if definitely not same type as the move",
            function()
            {
            ...
            it("Should return null if move could be same type as the target",
            function()
            {
            ...
            it("Should return false if move is the same type as the target",
            function()
            {
        ...
        describe("#assert()", function()
        {
            it("Should narrow hiddenpower and set #canHold()=true", function()
            {
                ...
                expect(user.traits.stats.hpType.possibleValues)
                    .to.include.keys("ice", "water");
                const move = reveal("hiddenpower");

                const reason = init(move);
                reason.assert();
                expect(mon.traits.stats.hpType.possibleValues)
            ...
            {
                mon.volatile.changeTypes(["ice", "???"]);
                expect(user.item.possibleValues)
                    .to.include.keys("icicleplate", "splashplate");
                const move = reveal("judgment");

                const reason = init(move);
                reason.assert();
                expect(user.item.possibleValues)
                    .to.not.have.keys("icicleplate");
                expect(reason.canHold()).to.be.true;
            });
            ...
            it("Should throw if can't be narrowed", function()
            {
                ...
                const reason = init(move);
                expect(() => reason.assert()).to.throw(Error,
                    "Move of type 'normal' cannot be asserted to not be of " +
                        "type [normal]");
            });
        });
        ...
        describe("#reject()", function()
        {
            ...
            it("Should narrow plate type and set #canHold()=false", function()
            {
                ...
                const reason = init(move);
                reason.reject();
                expect(user.item.possibleValues).to.have.keys("icicleplate");
                expect(reason.canHold()).to.be.false;
            });
            ...
            it("Should throw if can't be narrowed", function()
            {
                ...
                const reason = init(move);
                expect(() => reason.reject()).to.throw(Error,
                    "Move of type 'fire' cannot be asserted to be of type " +
                        "[normal]");
            });
        });
