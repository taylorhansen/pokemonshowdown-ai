...
export const test = () => describe("hp", function()
{
    ...
    describe("isAt1()", function()
    {
        ...
        describe("below 100 max hp (41-55)", function()
        {
            ...
            it("Should return null if definitely not at 1hp based on " +
                "percentage being 4%", function()
            {
            ...
            it("Should return chance reason if could be at 1hp based on " +
                "percentage being 3%", function()
            {
                const mon = sh.initActive("p2", lowMaxHp);
                mon.hp.set(3);
                ...
                expect([...reasons!].map(r => r.toString()))
                    .to.have.members(["ChanceReason()"])
            });

            it("Should return empty set if definitely at 1hp based on " +
                "percentage being 2%", function()
            {
                const mon = sh.initActive("p2", lowMaxHp);
                mon.hp.set(2);
