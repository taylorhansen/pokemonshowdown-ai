...
export const test = () =>
    describe("main", function () {
        ...
        const executor = setupOverrideExecutor(ictx);
        ...
        it("Should handle init and 1st turn and subsequent turns until game-over", async function () {
            ...
            agent.resolve();
            await expect(executor.executed()).to.eventually.equal("move 1");
            ...
            // choice.
            await expect(executor.executed()).to.eventually.equal("switch 2");
            executor.resolve(false /*i.e., accept the choice*/);
            await req4s;
