 ...
 * @param data Message(s) to be parses.
...
export function parsePSMessage(data: string, listener: MessageListener,
    logger = Logger.stdout): Promise<void>
{
    return new MessageParser(listener, logger).parse(data);
}
...
class MessageParser
{
    ...
    /** Object to which to send parsed messages. */
     ...
     * @param listener Object to which to send parsed messages.
     * @param logger Logger object.
     */
    constructor(listener: MessageListener, logger: Logger)
    {
        this.listener = listener;
        this.logger = logger;
    }

    /** Parses a message, executing constructor-provided callbacks. */
    public async parse(message: string): Promise<void>
    {
        ...
        if (message.startsWith(">"))
        {
            ...
            this.room = message.substring(1, pos);
        }
        else
        {
            pos = 0;
     ...
     */
    private dispatch<T extends MessageType>(type: T, message: Message<T>):
        Promise<void>
    {
        return this.listener.dispatch(type, message, this.room);
    }
